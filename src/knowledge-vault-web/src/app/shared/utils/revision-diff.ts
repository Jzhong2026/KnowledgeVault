...
/** Builds a line-oriented, side-by-side diff without adding a runtime dependency. */
export function buildRevisionDiff(oldContent: string, newContent: string): RevisionDiffBlock[] {
  ...
  for (const [matchingOldIndex, matchingNewIndex] of pairs) {
    appendChangedRows(rows, oldLines.slice(oldIndex, matchingOldIndex), newLines.slice(newIndex, matchingNewIndex), oldIndex, newIndex);
...
function appendChangedRows(
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    if (oldLine !== undefined && newLine !== undefined) {
      ...
      rows.push({ oldLineNumber: oldOffset + index + 1, newLineNumber: newOffset + index + 1, oldFragments, newFragments, kind: 'changed' });
    } else if (oldLine !== undefined) {
      rows.push({ oldLineNumber: oldOffset + index + 1, newLineNumber: null, oldFragments: [{ text: oldLine, kind: 'removed' }], newFragments: [], kind: 'removed' });
    } else if (newLine !== undefined) {
      rows.push({ oldLineNumber: null, newLineNumber: newOffset + index + 1, oldFragments: [], newFragments: [{ text: newLine, kind: 'added' }], kind: 'added' });
    }
  }
}
...
function collapseUnchangedRows(rows: RevisionDiffRow[]): RevisionDiffBlock[] {
  ...
  while (index < rows.length) {
    ...
    blocks.push({ kind: 'collapsed', rows: unchanged.slice(CONTEXT_LINE_COUNT, -CONTEXT_LINE_COUNT) });
...
/** Hirschberg's algorithm: O(mn) time and O(m+n) memory, suitable for long Markdown documents. */
function collectLcsPairs<T>(
  ...
  if (oldEnd - oldStart === 1) {
    ...
    if (match >= 0 && match < newEnd) {
      pairs.push([oldStart, match]);
    }
    return;
  }
